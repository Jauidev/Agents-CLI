...
The core (package `com.agentcli.terminal`):

- **`LinuxEngine.kt`** (object) — the engine. Extracts the bundled tarballs to `filesDir/{engine,rootfs}`, builds every `proot` invocation, and runs commands in the guest. Everything the guest does flows through `buildGuestCommand()` (proot arg prefix) + `applyGuestEnv()` (loader paths, `LD_LIBRARY_PATH`, `TMPDIR=/tmp`, etc.). `runInGuest`/`runInGuestStreaming` for one-shot commands. It also **writes files into the guest at runtime** — `ensureSessionScript()` (launcher + `.tmux.conf` + fake `xdg-open`) and `ensureAgentHooks()` (the `agent-notify` script + CLI hooks). That is the way to add guest-side behavior **without touching the rootfs asset or `ROOTFS_VERSION`**. `hostFileFor()` maps a guest path to the real Android path.
- **`LinuxService.kt`** — foreground `Service` that keeps a `ttyd` process alive per port/tab (map `port → Process`), holds a partial wake lock, can `restart()` a session with a new working directory, and hosts the `FileObserver` that turns guest notification events into Android notifications.
- **`MainActivity.kt`** — the terminal screen for one project. WebView + CLI selector (3 CLIs → 3 ports: 7681/7682/7683, defined in `AgentSession.kt`), special-keys bar, install/error status screen, and the browser-auto-open watcher.

Around them: `ProjectsActivity.kt` + `ProjectStore.kt` (launcher screen, recent projects), `EnvActivity.kt` (environment manager), `FilesActivity.kt` + `FileEditorActivity.kt` + `SyntaxHighlighter.kt` (file browser/editor), `LogConsole.kt` (shared live-log widget for long guest tasks — keeps only the tail and throttles UI updates, because painting thousands of lines into a `TextView` freezes the UI thread).
...
- **Special keys** (Esc/Tab/Ctrl/arrows) are injected by `evaluateJavascript` dispatching synthetic `keydown` events to xterm.js's `.xterm-helper-textarea` (xterm.js doesn't check `isTrusted`). Pasting (clipboard, attached image paths) uses a synthetic `paste` event with a `DataTransfer` instead, so newlines survive.
- **tmux windows are driven from inside the terminal**, never from Kotlin: the window chips send the tmux prefix (Ctrl+B) plus a key through that same synthetic-keydown path. A `tmux …` command issued via `runInGuest` would run in a *different* proot instance, and its client does not attach properly to the server traced by the first one (same reason each CLI already gets its own tmux socket via `-L`). tmux's status bar is on and `mouse on` makes tapping a window name switch to it.
- **Agent notifications** reuse the `xdg-open` trick: the guest script `agent-notify` appends a line to `/tmp/.agent-notify`, and `LinuxService`'s `FileObserver` raises the Android notification. It only fires while `/tmp/.agent-notify-armed` exists — `MainActivity` creates that flag in `onPause` and deletes it in `onResume`, so nothing pings you while the app is in front. Claude Code and Codex trigger it through their own hooks (`~/.claude/settings.json`, `~/.codex/config.toml`, merged non-destructively); Gemini and plain shells are covered by tmux's `alert-silence` hook.
